import { api, todayIso, shiftDate, type PlanDay } from "./api";
import { md } from "./md";
...
async function renderWeek(): Promise<void> {
  ...
  app.innerHTML = `<header class="top">${navHtml()}</header>
    <h1>Неделя ${esc(weekId.replace("week_", "").replace(/_/g, " "))}</h1>
...
async function renderLib(): Promise<void> {
  ...
  try {
    ...
    app.innerHTML = `<header class="top">${navHtml()}</header><h1>Библиотека</h1>
    ...
    app.querySelectorAll<HTMLButtonElement>("[data-lib]").forEach((b) => {
      b.onclick = async () => {
        const e = await api.libraryEntry(b.dataset.lib!);
...
async function renderSafe(): Promise<void> {
  ...
  try {
    ...
    app.innerHTML = `<header class="top">${navHtml()}</header><h1>${esc(doc.title)}</h1>
...
// init: дата из query ?date= для шаринга
